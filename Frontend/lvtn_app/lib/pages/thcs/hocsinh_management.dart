...
class THCSHocSinhManagement extends StatefulWidget {
  const THCSHocSinhManagement({super.key});

  @override
  State<THCSHocSinhManagement> createState() => _THCSHocSinhManagementState();
}

class _THCSHocSinhManagementState extends State<THCSHocSinhManagement> {
  bool isLoading = true;
  ...
  Future<void> fetchHocSinh() async {
    try {
      ...
      final res = await THCSApi.getListHocSinh(maTHCS: user['soDinhDanh']);
  ...
  Future<void> searchHocSinh(String keyword) async {
    try {
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      final user = authProvider.user ?? {};
      final res = await THCSApi.searchHocSinh(
        maTHCS: user['soDinhDanh'],
        keyword: keyword,
      );
      final data = (res.data['data'] as List)
          .map((e) => HocSinhFullData.fromJson(e))
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  if (hs.nguyenVong.isNotEmpty)
                                    Text(
                                      hs.nguyenVong.first.tenLopChuyen !=
                                                  null &&
                                              hs
                                                  .nguyenVong
                                                  .first
                                                  .tenLopChuyen!
                                                  .isNotEmpty
                                          ? "NV cao nhất: NV${hs.nguyenVong.first.thuTu}"
                                                "${hs.nguyenVong.first.nv2B == true ? 'b' : ''}"
                                                "${hs.nguyenVong.first.lopTiengPhap == true ? '-Pháp' : ''}"
                                                " - ${hs.nguyenVong.first.tenTHPT}"
                                                " (${hs.nguyenVong.first.tenLopChuyen})"
                                          : "NV cao nhất: NV${hs.nguyenVong.first.thuTu}"
                                                "${hs.nguyenVong.first.nv2B == true ? 'b' : ''}"
                                                "${hs.nguyenVong.first.lopTiengPhap == true ? '-Pháp' : ''}"
                                    ...
                                    const Text("NV cao nhất: (chưa xác định)"),
                                    ...
                                    children: [
                                      ElevatedButton.icon(
                                        ...
                                        onPressed: () => context.push(
                                          '/thcs/hocsinh/detail/${hs.thongTinHS.maHS}',
                                        ),
                                        icon: const Icon(
                                          Icons.arrow_forward,
                                          color: Colors.white,
