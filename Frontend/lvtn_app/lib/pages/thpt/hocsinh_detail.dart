...
class THPTHocSinhDetail extends StatefulWidget {
  final String maHS;
  const THPTHocSinhDetail({super.key, required this.maHS});

  @override
  State<THPTHocSinhDetail> createState() => _THPTHocSinhDetailState();
}

class _THPTHocSinhDetailState extends State<THPTHocSinhDetail> {
  bool isLoading = true;
  ...
  Future<void> fetchHocSinhDetail() async {
    try {
      final res = await THPTApi.getHocSinhDetail(maHS: widget.maHS);
  ...
  Widget build(BuildContext context) {
    ...
    final hs = hocSinh!;
    final List<NguyenVong?> nvArr = List.generate(2, (index) => null);
    for (final nv in hs.nguyenVong) {
      if (nv.thuTu >= 1 && nv.thuTu <= 2) {
        nvArr[nv.thuTu - 1] = nv;
      }
    }
    return SafeArea(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildRow(
                  ...
                  "${hs.thongTinHS.tenTHCSNgoaiTPCT} (${hs.thongTinHS.tenXaNgoaiTPCT}, ${hs.thongTinHS.tenTinhNgoaiTPCT})",
                ),
