...
export default function DeleteAllHocSinhTHCS({
  soDinhDanh,
  tenDinhDanh,
  onDeleted,
}) {
  ...
  const handleDelete = async () => {
    ...
    setIsError(false);
    try {
      await thcsApi.deleteAllHocSinh(soDinhDanh);
        ...
        >
          <div className="modal-content">
            <div className="modal-header bg-danger text-white justify-content-center">
              <h5 className="modal-title">Xóa toàn bộ học sinh của trường</h5>
            </div>
                  ...
                  {!message && (
                    <>
                      <div className="text-wrap">
                        Bạn có chắc chắn muốn xóa tất cả học sinh của trường{" "}
                        <strong>{tenDinhDanh}</strong> không?
                      </div>
                    </>
                  )}
