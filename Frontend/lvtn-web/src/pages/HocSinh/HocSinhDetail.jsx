...
import { useAuth, useImagePreview, useTrangThaiCapNhatHoSo } from "@/hooks";
import { hocsinhApi } from "@/api";
import { NoData, PageTitle, Spinner } from "@/components/common";
import { ExportWordHocSinh } from "@/components";
...
export function ButtonRow({ duocPhepCapNhat }) {
  ...
  return (
    <div className="text-center my-2">
      {duocPhepCapNhat && (
        <Link to="/hocsinh/update" className="btn btn-warning mx-1">
          Cập nhật thông tin
        </Link>
      )}
      <ExportWordHocSinh maHS={user.soDinhDanh} />
    </div>
  );
}
...
export function TableDetail() {
  ...
  useEffect(() => {
    const fetchHSDetail = async () => {
      setLoading(true);
      try {
        const res = await hocsinhApi.findHocSinh(maHS);
        const hs = res.data;
        setHocSinh(hs);
  ...
  if (loading || loadingTrangThai) return <Spinner />;

  if (hocSinh === null) {
    return (
      <>
        <ButtonRow duocPhepCapNhat={duocPhepCapNhat} />
        <NoData content="Không tồn tại thông tin học sinh!" />
      </>
    );
  }
        ...
        <table className="table mb-2">
          <tbody>
            ...
            <tr className="align-items-stretch">
              ...
              <td>
                {hocSinh.thongTinHS.tenTHCS} (
                {hocSinh.thongTinHS.tenPhuongXaTHCS})
              </td>
            </tr>
            <tr style={{ height: "40px" }}>
              <td colSpan="2" className="bg-success text-white fw-bold">
...
export default function HocSinhDetail() {
  useEffect(() => {
