...
import { formatDate, useImageUpload } from "@/hooks";
import { thptApi } from "@/api";

export function Title() {
  return (
    <div className="display-6 fw-bold text-center content mt-2 mb-2">
...
export function FormDetail() {
  ...
  const tachTienTo = (chuoi, danhSachLoai) => {
    if (!chuoi) return { loai: "", ten: "" };
    chuoi = chuoi.trim();
    for (const loai of danhSachLoai.sort((a, b) => b.length - a.length)) {
      if (chuoi.startsWith(loai + " ")) {
        return {
          loai,
          ten: chuoi.slice(loai.length).trim(),
        };
      }
    }
    return { loai: "", ten: chuoi };
  };
  const setHSData = useCallback(
    (hs) => {
      ...
      const { ten: tenTHCSName } = tachTienTo(hs.thongTinHS.tenTHCSNgoaiTPCT, [
        "THCS",
      ]);
      const { loai: loaiXaName, ten: tenXaName } = tachTienTo(
        hs.thongTinHS.tenXaNgoaiTPCT,
        ["Phường", "Xã", "Đặc khu"]
      );
      const { loai: loaiTinhName, ten: tenTinhName } = tachTienTo(
        hs.thongTinHS.tenTinhNgoaiTPCT,
        ["Tỉnh", "Thành phố"]
      );
      setTenTHCS(tenTHCSName);
      setLoaiXa(loaiXaName);
      setTenXa(tenXaName);
      setLoaiTinh(loaiTinhName);
      setTenTinh(tenTinhName);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await thptApi.updateHocSinh(user.soDinhDanh, maHS, formData);
      setSubmitting(false);
      setSuccess(true);
  ...
  useEffect(() => {
    const fetchHSDetail = async () => {
      setLoading(true);
      try {
        const res = await thptApi.findInfoHocSinh(maHS);
        let hs = res.data;
    ...
    reloadTrangThai();
    fetchHSDetail();
  }, [maHS, setHSData, reloadTrangThai]);
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="my-2 needs-validation">
...
export default function THPTUpdateHocSinh() {
  const { user } = useAuth();
